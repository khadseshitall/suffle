document.addEventListener("DOMContentLoaded", function() {
    const girlNames = [
        ...
        "priya ganeshrao Sadar",
        "Akshata maroti Khadse"
    ];
  
    const roomsContainer = document.getElementById("roomsContainer");
    const shuffleButton = document.getElementById("shuffleButton");
    const downloadShuffledButton = document.getElementById("downloadShuffledButton"); 
    ...
    function shuffleAndAssignRooms() {
      ...
      downloadShuffledButton.removeAttribute("disabled");
      downloadShuffledButton.addEventListener("click", function() {
        const shuffledListText = shuffledGirls.join("\n");
        const blob = new Blob([shuffledListText], { type: "text/plain" });
        const url = URL.createObjectURL(blob);
        const a = document.createElement("a");
        a.style.display = "none";
        a.href = url;
        a.download = "shuffled_girls.txt";
